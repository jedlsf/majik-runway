...
import { SUPABASE_DOMAIN_URL } from "./constants";
...
function createWindow() {
  const win = new BrowserWindow({
    ...
    icon: path.join(__dirname, "assets/Logo_MajikRunway_64px.png"),
  });
...
ipcMain.handle("import-data", async () => {
  ...
  if (!canceled && filePaths.length > 0) {
    ...
    const base64Content = await fs.readFile(filePaths[0], "utf-8");
    // Return the base64 string - the renderer will handle decryption
    return { base64Content, fileName: path.basename(filePaths[0]) };
  }
  return null;
});

ipcMain.handle("export-data", async (event, { base64Content, fileName }) => {
