...
import { Expense } from "@thezelijah/majik-runway";
...
interface CBaseExpenseProps {
  itemData: Expense;
  onPressed?: (itemData: Expense) => void;
...
const CBaseExpense: React.FC<CBaseExpenseProps> = ({
  itemData,
  onPressed,
  onDelete,
  canDelete = true,
  ...
  const handleOnPressed = () => {
    if (isDevEnvironment())
      console.log("Expense Item Pressed from Base: ", itemData);
  ...
  return (
    <RootContainer onClick={handleOnPressed}>
      ...
      <BodyContainer onClick={handleOnPressed}>
        <ColumnInfo>
          <ColumnMain>
            <ItemTitle>{itemData?.name ?? "Expense Item"}</ItemTitle>
            <SubColumnContainer>
              {itemData.isOneTime && (
                <RowTextItem
                  textKey="Price"
              ...
              {itemData.isCapital && (
                <RowTextItem
                  textKey="Depreciates in"
                  textValue={`${itemData.depreciationMonths} months`}
                  highlight={false}
                />
              )}
            </SubColumnContainer>
