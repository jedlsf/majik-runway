...
export class MajikRunway {
  ...
  private constructor(model: BusinessModel) {
    this.model = model;
  ...
  static parseFromJSON(json: string | object): MajikRunway {
    // If the input is a string, parse it as JSON
    const rawParse: BusinessModel =
      typeof json === "string"
        ? JSON.parse(json)
        : structuredClone
   ...
   *
   * @param cash - New initial cash amount to set. Must be a `MajikMoney` instance. Use MajikMoney.fromMajor(amount, currency) to create one.
   * @returns Chainable `MajikRunway` instance
   */
  public updateInitialCash(cash: MajikMoney): this {
    this.model.money = cash;
    return this;
  }
