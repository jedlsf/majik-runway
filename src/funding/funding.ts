...
export class FundingEvent {
   ...
   * @returns {object} - The plain object representation of the FundingEvent instance.
   */
  toJSON(): object {
    const preJSON = {
      ...this.state,
   ...
   * @returns FundingEvent instance
   */
  static parseFromJSON(json: string | object): FundingEvent {
    // If the input is a string, parse it as JSON
    const rawParse: FundingEventState =
      typeof json === "string"
        ? JSON.parse(json)
        : structuredClone
