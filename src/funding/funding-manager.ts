...
import { AmortizationEntry, FundingSummaryCache } from "./types";
import {
...
export class FundingManager {
   ...
   */
  generateFundingTimeSeries(months?: YYYYMM[]): FundingTimeSeriesTrace[] {
    const periodMonths =
      months ??
   ...
   * @returns {object} - The plain object representation of the FundingManager instance.
   */
  toJSON(): object {
    const preJSON = {
  ...
  static parseFromJSON(json: string | object): FundingManager {
    // If the input is a string, parse it as JSON
    const parsedData: FundingManager =
      typeof json === "string"
        ? JSON.parse(json)
        : structuredClone
