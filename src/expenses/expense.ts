...
import { ExpenseState, MonthlyAllocation } from "./types";
import {
  autogenerateID,
  monthsInPeriod,
  isValidYYYYMM,
...
export class Expense {
  ...
  /** Helper: inclusive month difference (e.g., Jan→Mar = 3) */
   ...
   * @returns {object} - The plain object representation of the Expense instance.
   */
  toJSON(): object {
    const preJSON = {
      ...this.state,
   ...
   * @returns Expense instance
   */
  static parseFromJSON(json: string | object): Expense {
    // If the input is a string, parse it as JSON

    const rawObject: Expense =
      typeof json === "string"
        ? JSON.parse(json)
        : structuredClone
