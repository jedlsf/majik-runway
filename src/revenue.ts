...
import { PeriodYYYYMM, RevenueID, YYYYMM } from "./types/types";
import {
  dateToYYYYMM,
  isValidYYYYMM,
  monthsArrayInPeriod,
...
export class RevenueStream<T extends RevenueItem = RevenueItem> {
  ...
  static parseFromJSON(json: string | object): RevenueStream {
    // If the input is a string, parse it as JSON
    const parsedData: RevenueStream =
      typeof json === "string"
        ? JSON.parse(json)
        : structuredClone
    ...
    const items: RevenueItem[] = (parsedData.items || []).map((item) => {
      switch (
