...
import { ExpenseBreakdownJSON, ExpenseSummaryCache } from "./types";
...
export class ExpenseBreakdown {
   ...
   * @returns {ExpenseBreakdownJSON} - The plain object representation of the ExpenseBreakdown instance.
   */
  toJSON(): ExpenseBreakdownJSON {
    const preJSON: ExpenseBreakdownJSON = {
      currency: this.currency,
      expenses: this.expenses.map((e) => e.toJSON()),
      period: this.period,
    };

    const serializedMoney: ExpenseBreakdownJSON = serializeMoney(preJSON);
  ...
  static parseFromJSON(json: string | ExpenseBreakdownJSON): ExpenseBreakdown {
    // If the input is a string, parse it as JSON
    const rawParse: ExpenseBreakdownJSON =
      typeof json === "string"
        ? JSON.parse(json)
        : structuredClone
